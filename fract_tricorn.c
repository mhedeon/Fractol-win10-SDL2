...
static Uint32	t_itter(t_fract *fract, t_z *z)
{
	int			i;
	double		tmp_re;
	double		tmp_im;
...
static void		tricorn(t_sdl *sdl)
{
	...
	while (++y < WIN_HEIGHT / THREADS + start)
	{
		...
		while (++x < WIN_WIDTH)
		{
			...
			sdl->buff[y * WIN_WIDTH + x] = t_itter(sdl->fract, &z);
		}
	}
}
...
void			draw_tricorn(t_sdl *sdl)
{
	...
	for (int i = 0; i < THREADS; i++)
	{
		...
		thread[i] = SDL_CreateThread(tricorn, "tricorn", (void *)&c_sdl[i]);
	}
	for (int i = 0; i < THREADS; i++)
	{
